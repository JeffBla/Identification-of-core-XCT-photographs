...
def getTargetCtImage(inDirname):
    ...
    return reader, files, dcmImage_CT


def getTargetMaskImg(inDirname):

    reader, files, dcmImage_CT = getTargetCtImage(inDirname)
            ...
            for idx, j in np.ndenumerate(Hu):
                ...
                if (checkInCircle(circle[0], circle[1],
                    ...
                    (cv.pointPolygonTest(big_contour,
                                         (idx[1], idx[0]), False) > 0)):
                    boolImg[idx[0], idx[1]] = True
                else:
                    boolImg[idx[0], idx[1]] = False

            maskImgSet.append(boolImg)
    return maskImgSet